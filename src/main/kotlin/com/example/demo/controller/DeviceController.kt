...
class DeviceController(@Autowired private val deviceService: DeviceService) {
    ...
    fun updateDeviceStatus(@RequestBody request: DeviceStatusRequestDTO): ResponseEntity<String> {
        ...
        return try {
            ...
            val updated = deviceService.updateDeviceStatus(request.chatroomId, request.deviceId, request.deviceStatus)
            ResponseEntity.ok("Device status updated successfully")
